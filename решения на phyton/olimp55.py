sin=[0,0.0175,0.0349,0.0523,0.0698,0.0872,0.1045,0.1219,0.1392,0.1564,0.1736,0.1908,0.2079,0.2250,0.2419,0.2588,0.2756,0.2924,0.3090,0.3256,0.3420,0.3584,0.3746,0.3907,0.4067,0.4226,0.4384,0.4540,0.4695,0.4848,0.5000,0.5150,0.5299,0.5446,0.5592,0.5736,0.5878,0.6018,0.6157,0.6293,0.6428,0.6561,0.6691,0.6820,0.6947,0.7071,0.7193,0.7314,0.7431,0.7547,0.7660,0.7771,0.7880,0.7986,0.8090,0.8192,0.8290,0.8387,0.8480,0.8572,0.8660,0.8746,0.8829,0.8910,0.8988,0.9063,0.9135,0.9205,0.9272,0.9336,0.9397,0.9455,0.9511,0.9563,0.9613,0.9659,0.9703,0.9744,0.9781,0.9816,0.9848,0.9877,0.9903,0.9925,0.9945,0.9962,0.9976,0.9986,0.9994,0.9998,1]
pi=3.1416
while True:
    xy1=input().split(' ')
    xy2=input().split(' ')
    r=int(input())
    s1=int(input())
    if s1<56773:
        xy1[0]=int(xy1[0])
        xy1[1]=int(xy1[1])
        xy2[0]=int(xy2[0])
        xy2[1]=int(xy2[1])
        c=(((xy1[0]-xy2[0])**2)+((xy1[1]-xy2[1])**2))**0.5
        if c<r*2:
            h=((r**2)-((c**2)/4))**0.5
            sinA=h/r
            for a in range(0,91):
                if sinA<sin[a]:
                    break
            a=a*2
            s=(pi*(r**2))*(2-(a/180))+(c*h)
        else:
            s=2*pi*(r**2)
        if s1<s:
            print('YES')
        else:
            print('NO')
    else:
        print('NO')
